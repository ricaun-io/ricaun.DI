...
namespace ricaun.DI
{
    ...
    /// </summary>
    /// <remarks><see cref="Dispose"/> will dispose all <see cref="IDisposable"/> scoped instances registered in the <see cref="Container"/>.</remarks>
    public class Container : IContainer, IDisposable
    {
        ...
        private object ResolveObject(Type contract, IDictionary<Type, Type> dic)
        {
            ...
            {
                throw new InvalidOperationException($"Container: {implementation.Name} has no available constructors. The container can not instantiate it.");
            }
        ...
        private void CheckForCircularDependencies(Type contract)
        {
            if (this.currentTypes.Contains(contract))
            {
                ...
                {
                    error = $"Container found circular dependency on {currentType.Name} trying to inject itself.";
                    throw new InvalidOperationException(error);
                }

                error = $"Container found circular dependency between {currentType.Name} and {contract.Name}.";
                throw new InvalidOperationException(error);
            }
        }
        ...
        private static void EnsureNonAbstractClass(Type type)
        {
            if (type.IsInterface)
            {
                throw new InvalidOperationException($"Can not add {type.Name} to Container because it is an interface. You need to provide a concrete type or provide an instance.");
            }
            if (type.IsAbstract)
            {
                throw new InvalidOperationException($"Can not add {type.Name} to Container because because it is an abstract type. You need to provide a concrete type or provide an instance.");
            }
        }

        private void EnsureDisposeScopedInstances()
        {
            if (this.disposeScopedInstances)
            {
                foreach (var scopedInstance in this.scopedInstances)
                {
                    if (scopedInstance.Value is IDisposable disposable)
                    {
                        try
                        {
                            if (this.canPrintToConsole)
                            {
                                Console.WriteLine("Container disposing: " + scopedInstance.Key.ToString());
                            }
                            disposable.Dispose();
                        }
                        catch { }
        ...
        {
            this.disposeScopedInstances = enabled;
        }
    }
}
